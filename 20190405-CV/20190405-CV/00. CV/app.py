    ...
    if "token" in session:   
        if request.method == "GET":
            ...
            if cv == None:
                ...
                return render_template("interfaceout.html",img_file=image,cvtitle = cvtitle, fullname = fullname, nominee = nominee, dateofbirth = dateofbirth,sex = sex, numberphone = numberphone, addressemail = addressemail, address = address, website = website, awareness = awareness, hobby = hobby, skillname = skillname, schoolname = schoolname, startschool = startschool, endschool = endschool, majors = majors, describe = describe, companyname = companyname, startcompany = startcompany, endcompany = endcompany, locationcompany = locationcompany, jobdescription = jobdescription, prize = prize, timeprize = timeprize,softskill = softskill, namecertificate = namecertificate, typecertificate = typecertificate, diffirent = diffirent, userstatus = True)
        elif request.method == "POST":
